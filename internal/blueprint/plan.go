...
func NewAction(step *Step, beforeResourceMap resourcesPkg.ResourceMap) *Action {
	action := &Action{
		Step:            step,
		ResourceActions: []ResourceAction{},
	}
	...
	for _, targetResource := range step.Resources() {
		beforeResource := beforeResourceMap.GetResourceWithSameTypeId(targetResource)
		if beforeResource == nil {
			...
			if reflect.DeepEqual(beforeResource, targetResource) {
				action.addResourceAction("✅", beforeResource, targetResource)
			} else {
				if resourcesPkg.GetResourceRemove(beforeResource) {
					...
					if resourcesPkg.GetResourceRemove(targetResource) {
						action.addResourceAction("🗑️", beforeResource, targetResource)
					} else {
						action.addResourceAction("🔄", beforeResource, targetResource)
					}
				}
