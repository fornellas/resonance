...
func NewHostStore(hst host.Host, path string) *HostStore {
	...
	return &HostStore{
		Host:          hst,
		basePath:      storePath,
		resourcesPath: filepath.Join(storePath, "resources"),
	}
}
...
func (s *HostStore) getResourcePath(resource resourcesPkg.Resource) string {
	hash := resourcesPkg.HashResource(resource)
	return fmt.Sprintf("%s.yaml", filepath.Join(s.resourcesPath, hash))
}

func (s *HostStore) SaveOriginalResource(ctx context.Context, resource resourcesPkg.Resource) error {
	path := s.getResourcePath(resource)
	...
	if hasFile {
		return errors.New("TODO SaveOriginalResource")
	} else {
		resourceStore := resourceStoreSchema{
			resourcesPkg.GetResourceTypeName(resource): map[string]resourcesPkg.Resource{
